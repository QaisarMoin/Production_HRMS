...
const NoticeLetterMaster = () => {
  ...
  const handleAdd = () => {
    setFormData({
      ...
      joining_date: '',
      notice_date: new Date().toISOString().split('T')[0],
      notice_reason: 'Failure to comply with organizational service level mandates.'
    });
    setIsModalOpen(true);
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this notice record?')) {
      try {
        await api.delete(`/notice-letters/${id}`);
        fetchData();
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/notice-letters', formData);
      setIsModalOpen(false);
      fetchData();
    } catch (err) {
      console.error('Error saving notice letter:', err);
    }
  };
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          <h3 className="text-lg font-black text-gray-900 flex items-center gap-2 m-0">
            ...
            Service Warnings & Notice Letters
          </h3>
          <p className="text-xs text-gray-500 mt-1 m-0">
            Enforce notice periods, document corporate non-compliance, and issue formal service notices with complete audit logging.
          </p>
        </div>
        <button
          onClick={handleAdd}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search notice files by code, recipient name..."
            value={searchTerm}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50/75">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Notice Issued Date</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Reason Outline</th>
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan="6" className="px-6 py-10 text-center text-xs text-gray-400">Loading notices log...</td>
                </tr>
                  ...
                  return (
                    <tr key={letRec._id} className="hover:bg-gray-50/40 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-500">
                        {new Date(letRec.notice_date).toLocaleDateString()}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-xs text-gray-500 max-w-[200px] truncate font-medium">
                        {letRec.notice_reason || 'N/A'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-xs font-medium">
                        <div className="flex space-x-3 items-center">
                          <a
                            href={`${api.defaults.baseURL || ''}/notice-letters/${letRec._id}/download`}
                            target="_blank"
                            rel="noreferrer"
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-10 text-center text-xs text-gray-400">No notice letters logged.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <Modal
        ...
        title="Issue Official Warning & Service Notice"
      >
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Notice Commencement Date</label>
              <input
                type="date"
          ...
          <div>
            <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Notice Explanation / Service Non-Compliance Reason</label>
            <textarea
              rows="4"
              value={formData.notice_reason}
              onChange={(e) => setFormData({ ...formData, notice_reason: e.target.value })}
              required
              className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-semibold"
              placeholder="Provide thorough descriptions regarding service warnings..."
            />
          </div>
...
export default NoticeLetterMaster;
